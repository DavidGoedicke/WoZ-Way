  ...
  switch (state) {
    ...
    case 't': //t like THINKING
      if (stateChange) {
        internalStateTracker = 1;
        stateChange = false;
        nextEvent = millis(); //inital delay for the think function
      }
      if (nextEvent <= millis()) {
        nextEvent = thinking(&internalStateTracker) + millis();
      }
      break;
